...
class IPConstants:
    ...
    DEFAULT_TOS = {TOS_PRECEDENCE: TOS_PRECEDENCE_ROUTINE, TOS_DELAY: TOS_DELAY_NORMAL,
                   TOS_THROUGHPUT: TOS_TROUGHPUT_NORMAL, TOS_RELIBILITY: TOS_RELIBILITY_NORMAL}
    DEFAULT_FLAGS = {FLAGS_RESERVED: FLAGS_RESERVED,
                     FLAGS_DF: FLAGS_DF_MAY_FRAGMENT, FLAGS_LF: FLAGS_LF_LAST_FRAGMENT}
    DEFAULT_TTL = 255
...
packet.set_source_address([l for l in ([ip for ip in socket.gethostbyname_ex(socket.gethostname())[2] if not ip.startswith("127.")][:1], [
